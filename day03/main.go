...
func (c coord) Distance(to coord) float64 {
	return math.Abs(float64(to.x-c.x)) + math.Abs(float64(to.y-c.y))
}

func (c coord) IsBoundedX(start, end coord) bool {
	xs := []int{start.x, end.x}
	sort.Ints(xs)

	if c.x < xs[0] || c.x > xs[1] {
		return false
	}
...
func (c coord) IsBoundedY(start, end coord) bool {
	ys := []int{start.y, end.y}
	sort.Ints(ys)

	if c.y < ys[0] || c.y > ys[1] {
		return false
	}
...
func newVectorFromString(start coord, vectorStr string) vector {
	direction := vectorStr[0]
	distance, _ := strconv.Atoi(vectorStr[1:])
	return vector{start, string(direction), distance}
}

func (v vector) end() coord {
	switch v.direction {
	case "U":
		return coord{x: v.start.x, y: v.start.y + v.distance}
	case "D":
		return coord{x: v.start.x, y: v.start.y - v.distance}
	case "L":
		return coord{x: v.start.x - v.distance, y: v.start.y}
	case "R":
		return coord{x: v.start.x + v.distance, y: v.start.y}
	default:
		return v.start
...
func (v vector) IsVertical() bool {
	return v.direction == "U" || v.direction == "D"
}

func (v vector) IsHorizontal() bool {
	return v.direction == "L" || v.direction == "R"
}

func (v vector) Intersection(other vector) (bool, coord) {
	var intersection coord

	if v.IsHorizontal() && other.IsVertical() && v.start.IsBoundedY(other.start, other.end()) && other.start.IsBoundedX(v.start, v.end()) {
		intersection.y = v.start.y
		intersection.x = other.start.x
		return true, intersection
	}

	if v.IsVertical() && other.IsHorizontal() && v.start.IsBoundedX(other.start, other.end()) && other.start.IsBoundedY(v.start, v.end()) {
		intersection.y = other.start.y
		intersection.x = v.start.x
		return true, intersection
	}
...
func tracePath(origin coord, path string) []vector {
	vectorStrs := strings.Split(path, ",")
	vectors := make([]vector, len(vectorStrs))
	for i, s := range vectorStrs {
		start := origin
		if i > 0 {
			start = vectors[i-1].end()
		}
		vectors[i] = newVectorFromString(start, s)
	}
	return vectors
}

func closestIntersection(path1Str, path2Str string) (coord, float64) {
	origin := coord{x: 0, y: 0}
	path1 := tracePath(origin, path1Str)
	path2 := tracePath(origin, path2Str)
	var closest coord
	var minDistance float64

	for _, v1 := range path1 {
		for _, v2 := range path2 {
			intersects, intersection := v1.Intersection(v2)
			if intersects && intersection.x != origin.x && intersection.y != origin.y {
...
func fewestSteps(path1Str, path2Str string) (coord, int) {
	origin := coord{x: 0, y: 0}
	path1 := tracePath(origin, path1Str)
	path2 := tracePath(origin, path2Str)
	var closest coord
	...
	for _, v1 := range path1 {
		...
		for _, v2 := range path2 {
			intersects, intersection := v1.Intersection(v2)
			if intersects && intersection.x != origin.x && intersection.y != origin.y {
				totalSteps := outerSteps + innerSteps + int(v2.start.Distance(intersection)) + int(v1.start.Distance(intersection))
				if numSteps == 0 || totalSteps < numSteps {
					numSteps = totalSteps
					closest = intersection
				}
			} else {
				innerSteps += v2.distance
			}
		}
		outerSteps += v1.distance
	}

	return closest, numSteps
}

func main() {
	closest, distance := closestIntersection(
		firstPath,
		secondPath,
	)
	fmt.Printf("closest point (%d, %d), distance = %f\n", closest.x, closest.y, distance)

	var steps int
	closest, steps = fewestSteps(
		firstPath,
		secondPath,
	)
	fmt.Printf("closest point (%d, %d), steps = %d\n", closest.x, closest.y, steps)
}
